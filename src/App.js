            ...
            <article className="media" key={i}>
              ...
              <div className="media-content">
                <div className="content">
                  <div>
                    <span ><a href = "#" className="green-text">{article.author.username}</a></span>
                    
                    <a className="article-link">
                      <p><em>{Date(article.updatedAt).slice(0,15)}</em></p>
...
class ShowTags extends React.Component {
  ...
  render(){
      return (
        ...
        {
          this.state.tags.map((tag, i) => (<button  key={i} className="tag button is-success is-medium">{tag}</button>))
        }
        </div>
      ) 
    }
...
function ShowTab(){
  return(
    <div className="tabs is-medium">
      <ul>
        <li className="is-active"><a href="#">Gloabal feed</a></li>
        <li><a href="#">Your feed</a></li>
        <li><a href="#">Recomended feed</a></li>
      </ul>
  </div>
  )
}
    ...
    <nav className="navbar" role="navigation" aria-label="main navigation">
      <div className="navbar-brand">
        ...
        <h1 className="title is-3 isPrimary">
          <Link className="navbar-item green-text" to='/'>Conduit</Link>
        </h1>
      </div>
      ...
      <div id="navbarBasicExample" className="navbar-menu">
        <div className="navbar-end">
          ...
          ? (
              <>
                <Link className="navbar-item" to='/newArticle'>New Post</Link>
                <Link className="navbar-item" to='/editUser'>Settings</Link>
                <Link className="navbar-item" to='/register'>Account</Link>
              </>
            ) 
          : ( <>
